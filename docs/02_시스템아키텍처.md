...
graph TB
    ...
    subgraph UserL["사용자·실시간 계층"]
        ...
        Community["💬 Community :8091"]
        Noti["🔔 Notification/Alert :8092\n알림 규칙 · FCM"]
    end
...
| Insight | 8085 | 모멘텀·실적·감성 종합 → 전망 카드 | **Kafka Streams**, MongoDB |
...
| Community | 8091 | 종목 토론방·게시글·댓글 | Outbox, PostgreSQL |
| Notification/Alert | 8092 | 알림 규칙(목표가 등) 평가, **FCM 푸시** | Kafka, FCM |
...
**② 인사이트 산출**
Insight가 `market.tick`(모멘텀) + `news.raw`(감성) + `fundamentals.raw`(실적)을 **Kafka Streams로 윈도우 집계·조인** → 종합 전망 스코어 → MongoDB 저장 + `insight.updated` → Realtime Gateway 화면 갱신 + Notification 알림.
...
| Namespace | 구성 |
|-----------|------|
| `stockpulse-app` | 13개 애플리케이션 서비스 + React 웹 |
...
- HPA는 `stockpulse-app`의 Gateway·Realtime·Insight에 적용.
